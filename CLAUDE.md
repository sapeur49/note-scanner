...
- **Web search tool**: `tools=[{"type": "web_search_20260209", "name": "web_search"}]` in `scan_notes()` in `app/main.py`. To disable, remove the `tools` param and revert response parsing back to `response.content[0].text`. To change when search triggers, edit the `**Web search:**` paragraph in `SCAN_PROMPT_BASE`. Note: custom `scan_prompt` in `user_settings` replaces `SCAN_PROMPT_BASE` — if a user sets a custom prompt, they lose the web-search instruction unless they include it themselves.

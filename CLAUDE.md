...
**Cache busting**: `?v=N` query strings on `app.js` and `style.css`. Bump when deploying JS/CSS changes — JS and CSS versions can differ (currently `style.css?v=68`, `app.js?v=88`). Update all nine HTML files: `index.html`, `results.html`, `notes.html`, `settings.html`, `notebooks.html`, `help.html` use relative paths; `share.html` and `published.html` use absolute paths (`/style.css?v=N`, `/app.js?v=N`) because their URL paths have two segments, which would break relative resolution. `landing.html` uses self-contained inline CSS — no version bump needed.
...
- **Multi-image ordering**: in `scan_notes()`, each image/document block sent to Claude is preceded by a `{"type": "text", "text": f"Image {i + 1}:"}` block (only when more than one file is uploaded) so Claude can reliably reference each photo's position. `SCAN_PROMPT_BASE` has a `**Multiple images:**` paragraph instructing Claude to number images per these labels (not its own count) when a multi-photo summary lists one item per image — without this, Claude tends to mislabel or repeat numbers (e.g. every item showing "#1") on scans with many photos.
...
`publish_note()` is idempotent (reuses existing token) and auto-generates a **random** slug (`_random_slug()` → `secrets.token_urlsafe(6)`, 8 chars) if none is set. Returns `{"share_token": ..., "slug": ...}`. `_slugify()` and `_make_slug()` are still used for title-derived slugs when the user opts in via the "Use title" button in the publish panel.
...
- `renderMarkdown(text)` — converts `## h2`, `### h3`, `- `/ `* ` lists, `1. ` ordered lists, `**bold**` to HTML; `[Name](url)` markdown links and bare `http`/`https` URLs both become `<a>` links (via `inlineFormat`); all other content becomes `<p>` wrapped. No external library.
...
| Grid (four squares) | Toggle gallery view for images | `#pub-gallery-toggle` on `published.html`; per-note `.sp-gallery-toggle` on `share.html` |
...
- **Web search tool**: `tools=[{"type": "web_search_20260209", "name": "web_search"}]` in `scan_notes()` in `app/main.py`. To disable, remove the `tools` param and revert response parsing back to `response.content[0].text`. To change when search triggers, edit the `**Web search:**` paragraph in `SCAN_PROMPT_BASE`. Citation behaviour is controlled by two further paragraphs in `SCAN_PROMPT_BASE`: `**Citation format:**` (requires bare `[Name](url)` markdown links with real URLs — no XML tags, no surrounding parentheses) and `**Do not narrate your research process**` (bans "web searches confirm…" meta-commentary). Note: custom `scan_prompt` in `user_settings` replaces `SCAN_PROMPT_BASE` — if a user sets a custom prompt, they lose all three web-search paragraphs unless they include them manually.
...
- **URL slug for published notes**: editable `#pub-slug` input in `results.html` publish panel, wrapped in `.pub-slug-row` flex container alongside `#pub-slug-use-title` button. Empty input → no preview (random slug auto-generated server-side on publish). "Use title" button fills slug from note title via `slugify()`. `savePublishOptions()` sends `slug` in PUT body when non-empty. `restorePublishOptions()` populates from `data.slug`. Lock/unlock functions also disable/enable `#pub-slug-use-title`. Random default: `_random_slug()` in `app/db.py` using `secrets.token_urlsafe(6)`.
...
- **Notebook access code**: set/clear via `PUT /api/notebooks/{id}` with `access_code` field. Hash stored in `notebooks.access_code_hash` via `set_notebook_access_code()` in `app/db.py`. Enforcement in `GET /api/published/{identifier}` when identifier is a notebook slug — use `_verify_access_code()` in `app/main.py` against the `X-Notebook-Access-Code` header. Frontend gate: `renderAccessGate()` inside `initPublished()` in `app.js`; verified code stored in `sessionStorage` under key `nb_access_{listToken}`. Notebook card UI: `renderAccessRow()` inside `createNbCard()` in `initNotebooks()` — shows lock button + "Access code: none/set" label; inline password form on click. Eye/eye-off toggle button (`eyeBtn`) next to the password input allows the owner to see what they're typing.
- **Gallery mode toggle** (image views on published pages): a visitor-facing, per-page-view toggle (not a `publish_options`/owner setting, not persisted anywhere — resets on reload) that switches images from the default "hero + small thumbnail strip" layout into a full-width vertical stack — each image at 100% width, natural (uncropped) aspect ratio, one after another. On `share.html`, `initShare()` inserts a `.sp-gallery-toggle` button into whichever `.sp-images` container is populated (only when a note has 2+ images); clicking toggles the `sp-images--gallery` class on that container, which relies on `display: contents` on `.sp-image-thumbs` (CSS in `style.css`) to lift the hero + thumbnail `<img>`s into one flex column — no DOM/JS restructuring needed, and the existing lightbox click handlers keep working unchanged. On `published.html`, the static `#pub-gallery-toggle` button (next to `#pub-search`) flips a `galleryMode` variable inside `initPublished()` and re-renders via `renderNotes(filteredNotes())` (same full-rerender pattern already used by the visibility filter); `createPubCard()` branches on `galleryMode` to build either the default markup (thumbnails after the snippet, inside `.pub-card-body`) or the gallery markup (hero + thumbnails wrapped together in `.pub-card-images`, which uses the same `display: contents` flex-column trick) — this branch exists because hero and thumbnails aren't siblings in the default DOM structure, so merging them into one stack requires different markup, not just a CSS class. (Note: an initial version used a CSS grid with `auto-fill`, which left small cropped-square thumbnails with empty space alongside them instead of filling the width — replaced with the full-width flex-column stack described here.)

...
**Cache busting**: `?v=N` query strings on `app.js` and `style.css`. Bump when deploying JS/CSS changes — JS and CSS versions can differ (currently `style.css?v=67`, `app.js?v=88`). Update all nine HTML files: `index.html`, `results.html`, `notes.html`, `settings.html`, `notebooks.html`, `help.html` use relative paths; `share.html` and `published.html` use absolute paths (`/style.css?v=N`, `/app.js?v=N`) because their URL paths have two segments, which would break relative resolution. `landing.html` uses self-contained inline CSS — no version bump needed.
...
- **Gallery mode toggle** (image views on published pages): a visitor-facing, per-page-view toggle (not a `publish_options`/owner setting, not persisted anywhere — resets on reload) that switches images from the default "hero + small thumbnail strip" layout into an even grid of equal-size tiles. On `share.html`, `initShare()` inserts a `.sp-gallery-toggle` button into whichever `.sp-images` container is populated (only when a note has 2+ images); clicking toggles the `sp-images--gallery` class on that container, which relies on `display: contents` on `.sp-image-thumbs` (CSS in `style.css`) to lift the hero + thumbnail `<img>`s into one CSS grid — no DOM/JS restructuring needed, and the existing lightbox click handlers keep working unchanged. On `published.html`, the static `#pub-gallery-toggle` button (next to `#pub-search`) flips a `galleryMode` variable inside `initPublished()` and re-renders via `renderNotes(filteredNotes())` (same full-rerender pattern already used by the visibility filter); `createPubCard()` branches on `galleryMode` to build either the default markup (thumbnails after the snippet, inside `.pub-card-body`) or the gallery markup (hero + thumbnails wrapped together in `.pub-card-images`, which uses the same `display: contents` grid trick) — this branch exists because hero and thumbnails aren't siblings in the default DOM structure, so merging them into one grid requires different markup, not just a CSS class.

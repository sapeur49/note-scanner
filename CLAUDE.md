...
**Cache busting**: `?v=N` query strings on `app.js` and `style.css`. Bump when deploying JS/CSS changes — JS and CSS versions can differ (currently `style.css?v=53`, `app.js?v=62`). Update all nine HTML files: `index.html`, `results.html`, `notes.html`, `settings.html`, `notebooks.html`, `help.html` use relative paths; `share.html` and `published.html` use absolute paths (`/style.css?v=N`, `/app.js?v=N`) because their URL paths have two segments, which would break relative resolution. `landing.html` uses self-contained inline CSS — no version bump needed.
...
- **Notebook URL slugs**: `slug` column on `notebooks` table (added via `_migrate_schema`). `create_notebook()` auto-generates via `_make_notebook_slug(_slugify(title))`. `PUT /api/notebooks/{id}` accepts optional `slug` in payload. `GET /api/published/{list_token}?nb={slug}` resolves slug via `get_notebook_by_slug()` and returns `activeNotebook: {id, title, slug}` in the response. `initPublished()` reads `?nb=` URL param, passes to API, and uses `data.activeNotebook.title` as page header when present. Changing the notebook dropdown in `initPublished()` calls `history.replaceState` with updated `?nb=`. Notebook cards on `/notebooks` show a shareable URL row with copy button and inline slug editor (saved via `PUT /api/notebooks/{id}` with `{title, slug}` body); URL row only rendered when `window._pubListToken` is set (populated from `/api/settings` at page load).

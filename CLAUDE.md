...
**Cache busting**: `?v=N` query strings on `app.js` and `style.css`. Bump when deploying JS/CSS changes — JS and CSS versions can differ (currently `style.css?v=54`, `app.js?v=63`). Update all nine HTML files: `index.html`, `results.html`, `notes.html`, `settings.html`, `notebooks.html`, `help.html` use relative paths; `share.html` and `published.html` use absolute paths (`/style.css?v=N`, `/app.js?v=N`) because their URL paths have two segments, which would break relative resolution. `landing.html` uses self-contained inline CSS — no version bump needed.
...
- **Notebook URL slugs**: `slug` column on `notebooks` table (added via `_migrate_schema`). Notebooks start with `slug=NULL` (no public URL). `PUT /api/notebooks/{id}` accepts optional `slug` in payload; `slug=""` clears to NULL. `GET /api/published/{list_token}?nb={slug}` resolves slug via `get_notebook_by_slug()` and returns `activeNotebook: {id, title, slug}`. `initPublished()` reads `?nb=` URL param and uses `data.activeNotebook.title` as page header when present; dropdown change updates title + URL via `history.replaceState`. Notebook cards on `/notebooks` have a globe toggle button (grey = no URL, accent = active); clicking grey globe enables (sends `PUT` with title only → auto-slug); clicking active globe disables (sends `slug: ""`). URL row (shown when slug set) has `?nb=<slug>` link, inline editable slug input (auto-saves 800ms debounce), and copy button. `window._pubListToken` set from `/api/settings` at page load; URL row hidden if absent.

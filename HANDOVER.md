...
Cache-busters: **`style.css?v=54`**, **`app.js?v=63`** across nine HTML files (relative paths: `index.html`, `results.html`, `notes.html`, `settings.html`, `notebooks.html`, `help.html`; absolute paths `/style.css?vN`, `/app.js?vN`: `share.html`, `published.html`). `landing.html` uses self-contained inline CSS — no version bump needed.
...
|---|---|---|---|
| 0 | **Notebook public URL redesign** — Notebooks start with no public URL. Globe button on each card: grey = disabled, accent = active. Clicking grey globe enables (auto-generates slug); clicking active globe disables with confirm (clears slug). URL row shows `?nb=<slug>` link, inline editable slug input (auto-saves 800ms after typing), and copy button. Backend: `update_notebook` handles `slug=""` → NULL; `create_notebook` no longer auto-generates slug. | v63 / v54 | `app/db.py` (`update_notebook` slug="" handling, `create_notebook`), `app.js` (`createNbCard` globe toggle + `renderSlugRow`), `style.css` (`.nb-globe-active`, `.nb-slug-input`) |

...
Cache-busters: **`style.css?v=54`**, **`app.js?v=65`** across nine HTML files (relative paths: `index.html`, `results.html`, `notes.html`, `settings.html`, `notebooks.html`, `help.html`; absolute paths `/style.css?vN`, `/app.js?vN`: `share.html`, `published.html`). `landing.html` uses self-contained inline CSS — no version bump needed.

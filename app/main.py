...
**Citation format:** When you draw on web search results, do NOT insert source links inline within body text. Write all prose naturally with no links, bracketed citations, or superscripts interrupting sentences. At the very end of the `additional_notes` field only, if web search was used, append a `### Sources` section listing each source actually referenced. Each source is one line in this exact format — a markdown link where the link text is the publication name and a brief article title or description, and the URL is hidden inside the parentheses:

### Sources
- [Publication Name: Brief article title or description](https://real-url-from-search-result)

The link text (inside `[…]`) must be concise enough to read as a single line on mobile — aim for under 80 characters. The raw URL must ONLY appear inside `(…)` and must never be written out as visible text. Do not write bare URLs anywhere in the Sources list. Omit the `### Sources` section entirely if no web search was used. Never add a Sources section to `summary` or `transcription`. Use only real URLs from the search results — never fabricate a URL. Your entire response must be clean text and markdown only, with no XML, HTML, or `<cite>` tags of any kind.
